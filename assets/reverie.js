(() => {
 ...
 const slots = photos.map((photo, i) => {
   ...
   photo.before(slot);slot.append(photo);
 ...
 function sync(){
   ...
   previous.disabled=active===0;next.disabled=active===slots.length-1;
 }
 ...
 album.addEventListener('scroll',()=>{cancelAnimationFrame(frame);frame=requestAnimationFrame(sync);},{passive:true});
 ...
 photos.forEach((photo,i)=>photo.addEventListener('focus',()=>center(i)));
